...
def test_load_requires_model(monkeypatch):
...
def test_load_requires_model_provider_if_model_not_prefixed(monkeypatch):
...
def test_load_parses_provider_and_model_from_model_env(monkeypatch):
    monkeypatch.setenv("MODEL", "ollama:llama3")
    monkeypatch.delenv("MODEL_PROVIDER", raising=False)
    monkeypatch.delenv("BASE_URL", raising=False)

    config = ChatModelClientConfig.load(client_name="TestClient")
...
def test_load_uses_model_provider_env_over_model_prefix(monkeypatch):
    monkeypatch.setenv("MODEL", "llama3")
...
def test_load_reads_base_url(monkeypatch):
    monkeypatch.setenv("MODEL", "ollama:llama3")
    monkeypatch.delenv("MODEL_PROVIDER", raising=False)
...
def test_load_invalid_provider_in_model_prefix_raises_validation(
    monkeypatch,
):
...
def test_load_invalid_provider_env_raises_validation(monkeypatch):
    monkeypatch.setenv("MODEL", "llama3")
