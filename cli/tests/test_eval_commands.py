...
def test_load_eval_config_rejects_unknown_target(tmp_path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    root = Path.cwd()
    _write_minimal_agent_root(root)
    ...
    try:
        ...
        assert "evaluation.target must be one of" in str(exc)
    else:
        raise AssertionError("Expected unknown target to fail load")


def test_load_eval_config_remote_rejects_multiple_models(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    root = Path.cwd()
    ...
    try:
        ...
        assert "single model entry" in str(exc)
    else:
        raise AssertionError("Expected remote multi-model to fail load")


def _remote_config(root: Path, dataset: Path, output_dir: Path) -> EvalConfig:
    return EvalConfig(
        dataset=dataset.resolve(),
        output_dir=output_dir.resolve(),
        agent_url="http://agent.remote:9900",
        agent_startup_timeout_s=15,
        agent_shutdown_timeout_s=5,
        k=1,
        qualitative=False,
...
def test_eval_run_remote_skips_lifecycle(tmp_path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    root = Path.cwd()
    # Note: no agent.json/config.yaml/pyproject.toml — remote must not require them.
    eval_input = root / "eval" / "input"
    eval_output = root / "eval" / "output"
    eval_input.mkdir(parents=True, exist_ok=True)
    eval_output.mkdir(parents=True, exist_ok=True)
    ...
    config = _remote_config(root, dataset, eval_output)
    captured: dict[str, object] = {}

    def fake_load_eval_config(agent_root: Path, config_path: Path):
        return config

    def boom_popen(*args, **kwargs):  # noqa: ANN002, ANN003
        raise AssertionError("remote target must not launch a process")

    def fake_wait_remote(agent_url: str, timeout_s: int) -> None:
        captured["remote_url"] = agent_url
        captured["remote_timeout"] = timeout_s

    def fake_run_eval_orchestrator(**kwargs):  # noqa: ANN003
        captured["runner_kwargs"] = kwargs

    monkeypatch.setattr("eval.commands.load_eval_config", fake_load_eval_config)
    monkeypatch.setattr("eval.commands.subprocess.Popen", boom_popen)
    monkeypatch.setattr(
        "eval.commands._wait_for_remote_agent", fake_wait_remote
    )
    monkeypatch.setattr(
        "eval.commands._run_eval_orchestrator", fake_run_eval_orchestrator
    )
    ...
    assert captured["remote_url"] == "http://agent.remote:9900"
    assert captured["remote_timeout"] == 15
    assert captured["runner_kwargs"]["agent_url"] == "http://agent.remote:9900"
    assert captured["runner_kwargs"]["model"] == "gpt-4.1-mini"


def test_eval_run_docker_starts_container(tmp_path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    root = Path.cwd()
    eval_input = root / "eval" / "input"
    eval_output = root / "eval" / "output"
    ...
    dataset = eval_input / "tasks.json"
    dataset.write_text("[]\n", encoding="utf-8")
    # Secrets the container should receive via --env-file.
    ...
    config = EvalConfig(
        ...
        judge=None,
        target="docker",
        image="hub.example.com/labs/agent:latest",
    )
    ...
    def fake_run(cmd, **kwargs):  # noqa: ANN001, ANN003
        captured.setdefault("docker_cmds", []).append(cmd)
    ...
    def fake_wait_container(agent_url, timeout_s, container_name):  # noqa: ANN001
    ...
    monkeypatch.setattr(
        "eval.commands._wait_for_agent_container", fake_wait_container
    )
    monkeypatch.setattr(
        "eval.commands._run_eval_orchestrator", fake_run_eval_orchestrator
    )
    ...
    docker_cmds = captured["docker_cmds"]
    run_cmd = next(c for c in docker_cmds if c[:3] == ["docker", "run", "-d"])
    assert "hub.example.com/labs/agent:latest" in run_cmd
    assert "--network" in run_cmd and "host" in run_cmd
    # secrets forwarded via the agent .env
    assert "--env-file" in run_cmd
    # model selection forwarded explicitly
    assert "MODEL=gpt-4.1-mini" in run_cmd
    assert "MODEL_PROVIDER=openai" in run_cmd
    # container was torn down
    assert any(c[:3] == ["docker", "stop", "-t"] for c in docker_cmds)
