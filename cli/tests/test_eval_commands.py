...
def test_eval_show_requires_eval_yaml() -> None:
    with runner.isolated_filesystem():
        root = Path.cwd()
        _write_minimal_agent_root(root)
...
def test_eval_show_prints_resolved_config(monkeypatch) -> None:
    with runner.isolated_filesystem():
        root = Path.cwd()
        _write_minimal_agent_root(root)
...
def test_load_eval_config_requires_judge_when_qualitative_is_true() -> None:
    with runner.isolated_filesystem():
        root = Path.cwd()
        _write_minimal_agent_root(root)
...
def test_judge_prompts_parsed() -> None:
    with runner.isolated_filesystem():
        root = Path.cwd()
        _write_minimal_agent_root(root)
...
def test_judge_prompts_partial_ok() -> None:
    with runner.isolated_filesystem():
        root = Path.cwd()
        _write_minimal_agent_root(root)
        eval_yaml = _write_eval_yaml_with_prompts(
...
def test_judge_prompts_overflow_errors() -> None:
    with runner.isolated_filesystem():
        root = Path.cwd()
        _write_minimal_agent_root(root)
        overflow = "x" * 1001
...
def test_judge_prompts_unknown_key_errors() -> None:
    with runner.isolated_filesystem():
        root = Path.cwd()
        _write_minimal_agent_root(root)
        eval_yaml = _write_eval_yaml_with_prompts(
...
def test_judge_prompts_absent_is_empty_dict() -> None:
    with runner.isolated_filesystem():
        root = Path.cwd()
        _write_minimal_agent_root(root)
...
def test_eval_plot_without_filter_keeps_all_per_task_charts() -> None:
    with runner.isolated_filesystem():
        out = Path.cwd() / "out"
