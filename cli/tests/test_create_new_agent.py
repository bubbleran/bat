...
def test_create_new_agent_pyproject_name_from_camel_case(
    ...
    assert result.exit_code == 0
    # The created directory is always lowercased; directory-derived names
    # (here the pyproject name) follow that lowercase folder name.
    pyproject_content = Path("rngagent", "pyproject.toml").read_text(
        encoding="utf-8"
    )
    assert 'name = "rngagent"' in pyproject_content


def test_create_new_agent_lowercases_directory_but_keeps_class_casing(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    result = runner.invoke(app, ["init", "agent", "SLA-agent"])
    ...
    root = Path("sla-agent")
    assert root.exists()
    assert not Path("SLA-agent").exists()

    # ... while the State/Graph class names keep the original casing.
    graph_content = (root / "src" / "graph.py").read_text(encoding="utf-8")
    assert "class SLAAgentState(AgentState):" in graph_content
    assert "class SLAAgentGraph(AgentGraph):" in graph_content

    # Directory-derived names follow the lowercase folder name.
    agent_json_content = (root / "agent.json").read_text(encoding="utf-8")
